...
class MrJump extends JComponentWithEvents{
  ...
  //block colors (variable to level)
  ...
  private Color bodyColor = Color.red; //(variable to level)
  private Color skinColor = Color.white; //(variable to select)
  ...
  public void start(){
    ...
    if (!startScreen){//start game when start screen is not showing
      ...
      stageScroll = 0;
      runnerY = 10;
      deathPaintCounter = 0; //drawing next iteration after death
    }
  }
  ...
  public void getLevelPercent(){
    ...
    if (levelPercent > 101){ //die if past goal 
      die();
    }
  }
  ...
  public void paintStage(Graphics2D page){
    for (int row = 0; row < STAGE_ROW; row ++){
      //start at where screen shows, end where screen ends
      for (int col = (stageScroll/blockSize); 
